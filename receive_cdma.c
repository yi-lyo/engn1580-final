/*
 * receive_cdma.c — CDMA-enhanced PSK audio receiver with SDL2 visualization
 *
 * Modulation: BPSK chips with CDMA despreading (Walsh-Hadamard codes)
 * ─────────────────────────────────────────────────────────────────────
 * Receives BPSK chips at 0° or 180° phase.
 * Accumulates SF chips per symbol using soft-decision decoding.
 * Despreads using Walsh-Hadamard correlation (cdma_despread_soft).
 * Recovers data symbols carrying log2(SF) bits each.
 *
 * Processing Gain
 * ───────────────
 ...
 *
 * The transmitter prepends PREAMBLE_CHIPS phase-0° chips before data.
 ...
 *   DECODING    — offset known; accumulating chips and despreading symbols
 *
 * Architecture
 * ────────────
 *   PortAudio thread   — projects audio onto carrier phasor (fast O(N))
 *                        copies samples + complex carrier value to shared buf
 *   Main thread        — computes 100-bin DFT, decodes BPSK chips, accumulates
 *                        SF chips, despreads to recover symbols, renders
 *
 * Command-line flags
 * ──────────────────
 *   (none)              auto-select best input device
 *   -e                  enable FEC decoding (must match transmitter)
 *   -d <depth>          FEC interleave depth
 *   -o <file>           write decoded bytes to file when carrier drops
 *   --device <index>    force a specific PortAudio device index
                             ...
                             int timing_locked, int sym_window)
{
    ...
    {
        char title[32];
        snprintf(title, sizeof(title), "BPSK Constellation (Chips)");
        /* Rough centering */
...
static void render_frame(SDL_Renderer *ren, TTF_Font *font,
                          ...
                          int fec_depth,
                          /* CDMA */
                          float correlation, float chip_error_rate, int chip_count)
{
    ...
    /* ── CDMA decode readout ───────────────────────────────────────── */
    {
        ...
        /* CDMA status: chip/correlation/error rate */
        {
            SDL_Color sc =
            ...
            if (rx_state < 2) {
                ...
                snprintf(buf, sizeof(buf),
                         "Chip: %d  Chips: %d/%d  Corr: %.1f  ChipER: %.1f%%",
                         current_sym, chip_count, CDMA_SPREADING_FACTOR,
                         correlation, chip_error_rate * 100.0f);
            }
     ...
     * ─────────────────────────────────────────────────────────────── */
    {
        ...
        float pg_db = cdma_calculate_processing_gain_db(CDMA_SPREADING_FACTOR);
        char cdma_buf[64];
        snprintf(cdma_buf, sizeof(cdma_buf),
                 "CDMA: SF=%d (PG %.1f dB)",
                 CDMA_SPREADING_FACTOR, pg_db);
        SDL_Color cc = {100, 160, 220, 255};
        draw_text(ren, font, cdma_buf, x, STAT_Y + 4, cc);
        x += 180;
     ...
     * ─────────────────────────────────────────────────────────────── */
    {
        ...
        if (timing_locked && rx_state == 2) {
            float max_err_deg = 90.0f;  /* BPSK: ±90° max error (180/M where M=2) */
...
{
    /* ── argument parsing ─────────────────────────────────────────── */
    int            M            = 2;   /* BPSK for chip transmission */
    int            fec_enabled  = 0;
    ...
    for (int a = 1; a < argc; a++) {
        if (strcmp(argv[a], "-e") == 0) {
            ...
            fprintf(stderr, "Unknown argument: %s\n"
                    "Usage: %s [-e] [-d DEPTH] [-o OUTPUT_FILE]\n"
                    "          [--device <idx>] [-n <substr>] [--list-devices]\n"
                    "  -e        enable FEC decoding (must match transmitter)\n"
                    "  -d DEPTH  FEC interleave depth (default %d)\n"
                    "  -o FILE   write decoded bytes to FILE when carrier drops\n",
    ...
    fprintf(stderr,
            "CDMA Receiver — BPSK chips, SF=%d, carrier bin %d (~%.0f Hz)\n"
            "  Bits/symbol: %d, Processing gain: %.1f dB\n",
            SF, CARRIER_BIN,
            (float)CARRIER_BIN * SAMPLE_RATE / FRAMES_PER_BUFFER,
            CDMA_BITS_PER_SYMBOL, processing_gain_db);
...
#define PREVIEW_MAX 8192
        ...
        /* CDMA chip accumulation and despreading state */
        float chip_buffer[CDMA_SPREADING_FACTOR];  /* soft chips for correlation */
        int   chip_count   = 0;       /* chips accumulated so far              */
        float correlation  = 0.0f;    /* last despread correlation value       */
        float chip_error_rate = 0.0f; /* estimated chip error rate             */
        ...
        while (running && Pa_IsStreamActive(stream)) {
            ...
            if (timing_locked && !prev_timing_locked) {
                ...
                /* Reset CDMA chip buffer on first timing lock */
                chip_count = 0;
                correlation = 0.0f;
                chip_error_rate = 0.0f;
                memset(chip_buffer, 0, sizeof(chip_buffer));
            }
           ...
           if (snr_db >= CARRIER_SNR_MIN_DB || carrier_mag > SIG_THRESHOLD) {
                ...
                if (rx_state == 0) {
                    ...
                    /* Reset CDMA chip buffer on new carrier detection */
                    chip_count = 0;
                    correlation = 0.0f;
                    chip_error_rate = 0.0f;
                    memset(chip_buffer, 0, sizeof(chip_buffer));
                ...
                if (rx_state == 2) {
                    ...
                    /* CDMA: Convert phase to soft chip value */
                    float soft_chip = cdma_psk_phase_to_chip_soft(corrected);
                    
                    /* Map to nearest BPSK symbol for display/PLL */
                    int sym = (int)roundf(
                        (float)M * corrected / (2.0f * (float)M_PI));
                    int candidate_sym  = ((sym % M) + M) % M;
                    int candidate_bits = candidate_sym;  /* BPSK: no Gray coding needed */

                    if (!is_boundary
                        && (!timing_locked || sym_window == 2)
                        && skip_after_lock == 0) {
                        /* Safe window: accept this chip */
                        current_sym  = candidate_sym;
                        current_bits = candidate_bits;

                        /* Accumulate chip into buffer */
                        chip_buffer[chip_count++] = soft_chip;
                        
                        /* When we have SF chips, despread to get symbol */
                        if (chip_count >= CDMA_SPREADING_FACTOR) {
                            int despread_sym = cdma_despread_soft(chip_buffer, 
                                                                  CDMA_SPREADING_FACTOR, 
                                                                  &correlation);
                            
                            if (despread_sym >= 0) {
                                /* Valid symbol recovered */
                                chip_error_rate = cdma_estimate_chip_error_rate(
                                    (int)correlation, CDMA_SPREADING_FACTOR);
                                
                                /* Convert CDMA symbol to bytes and accumulate */
                                size_t n_syms = 0;
                                uint8_t sym_byte = (uint8_t)despread_sym;
                                uint8_t *sym_bytes = cdma_symbols_to_bytes(&sym_byte, 1, &n_syms);
                                
                                if (sym_bytes && n_syms > 0) {
                                    /* Append to accumulator */
                                    int bps = CDMA_BITS_PER_SYMBOL;
                                    size_t bits_now = fec_sym_bits;
                            ...
                            /* Reset chip buffer for next symbol */
                            chip_count = 0;
                            memset(chip_buffer, 0, sizeof(chip_buffer));
                        }
                         ...
                         * for the decided BPSK chip symbol.           */
                ...
                if (rx_state == 1) {
                    ...
                    /* Reset CDMA chip buffer when aborting calibration */
                    chip_count = 0;
                    correlation = 0.0f;
                    chip_error_rate = 0.0f;
                    memset(chip_buffer, 0, sizeof(chip_buffer));
             ...
             *                           recovered bytes.
             */
            /* CDMA: Symbol accumulation is now handled inside the 
             * despreading logic above, not here. This block is removed
            ...
            if (snr_db < CARRIER_SNR_MIN_DB && carrier_mag <= SIG_THRESHOLD
                ...
                /* Reset CDMA chip accumulation state */
                chip_count         = 0;
                correlation        = 0.0f;
                chip_error_rate    = 0.0f;
                memset(chip_buffer, 0, sizeof(chip_buffer));
            ...
            render_frame(ren, font, magnitudes, peaks,
                         carrier_mag, carrier_phase_deg,
                         current_sym, current_bits, rx_state,
                         sym_window, is_boundary, timing_locked,
                         pll_error_deg,
                         snr_db, interference, intf_bin,
                         fec_enabled, fec_corrections, fec_depth,
                         correlation, chip_error_rate, chip_count);
