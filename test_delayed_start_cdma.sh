...
./receive_cdma --device 12 -e -d 32 -o "$OUTPUT_FILE" &
RECEIVER_PID=$!
...
echo "$TEST_MESSAGE" | ./transmit_cdma -e -d 32 > /dev/null 2>&1
...
# Launch receiver in background with file output
./receive_cdma --device 12 -e -d 32 -o "$OUTPUT_FILE" > /dev/null 2>&1 &
RECEIVER_PID=$!
